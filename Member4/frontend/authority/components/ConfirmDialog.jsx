...
export default function ConfirmDialog({ open, title, description, confirmLabel = 'Confirm', onConfirm, onCancel, danger = false }) {
  ...
  return (
    <div
      role="dialog"
      aria-modal="true"
      aria-labelledby="cm-confirm-dialog-title"
      className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm px-4"
      onKeyDown={(e) => { if (e.key === 'Escape') onCancel(); }}
    >
      <div className="cm-glass-panel w-full max-w-sm p-6">
        <div id="cm-confirm-dialog-title" className="font-semibold text-[15px] mb-2">{title}</div>
        <div className="text-[13px] text-[color:var(--cm-text-secondary)] mb-6">{description}</div>
        <div className="flex justify-end gap-2">
          <button
            onClick={onCancel}
            autoFocus
            className="cm-focusable px-3 py-2 rounded-md text-[13px] border border-[color:var(--cm-border-strong)] hover:bg-[color:var(--cm-bg-panel-raised)]"
          >
            Cancel
          </button>
          <button
            onClick={onConfirm}
            className="cm-focusable px-3 py-2 rounded-md text-[13px] font-medium"
            style={{
              color: '#0f2136',
